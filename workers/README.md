...
2. **재배포가 겹친다.** 연락처를 고치려고 배포했다가 상담 예약이 깨지면 하필
   그날이 상담 오픈일일 수 있다(반대도 마찬가지). 배포가 손으로 붙여넣기라
   되돌리기도 붙여넣기뿐이다 — 공지를 손보다 학생 사진이 죽는 일은 없어야 한다.

대가로 공통 코드(서명·Firestore REST·CORS)가 파일마다 중복됩니다. 거의 바뀌지 않는
인프라 코드라 중복을 감수했습니다. **한쪽을 고치면 다른 쪽도 봐야 합니다.**
(`notice-api`는 Firestore를 안 건드려서 서명·REST 쪽은 아예 안 들고 왔습니다.)
...
# 전체 공지 이미지 (`notice-api`)

현황판 맨 위 **전체 공지**에 붙는 그림만 담습니다. 공지 **글**은 Firestore
(`appNotice/board`, test는 `appNotice/board-test`)에 그대로 들어가고 이 워커를
거치지 않습니다.

이름은 `notice-api` 지만 **업무캘린더 메모의 그림도 같이 담습니다**(2026-09-10부터).
자리를 갈라 두었을 뿐 한 워커·한 버킷입니다.

| 키 모양 | 무엇 | 올리기·지우기 |
|---|---|---|
| `notices/<배포>/<공지id>/<파일id>.jpg` | 전체 공지 | **관리자만** |
| `tasks/<배포>/<올린이uid>/<일정id>/<파일id>.jpg` | 업무캘린더 메모 | **교사 누구나**, 자기 칸만 |

- `GET  /img?k=<key>` — 이미지 보기 (교사면 누구나)
- `POST {action:'put',  kind:'notice'|'task'}` — 올리기 → `{ key }`
- `POST {action:'del',  keys:[...]}` — 지우기 (키마다 권한을 따로 따집니다)
- `POST {action:'sweep', kind:'notice'|'task'}` — 안 쓰이는 이미지 청소

### 캘린더 쪽에 uid 칸이 하나 더 있는 이유

공지는 관리자가 **전부** 볼 수 있어서, 청소할 때 "지금 쓰이는 그림"의 목록이
완전합니다. 배포 전체를 훑어도 됩니다.

일정은 다릅니다. 각자 **자기 것만** 봅니다. 같은 방식으로 훑으면 남이 쓰는
그림이 "아무도 안 쓰는 것"으로 보여 **통째로 지워집니다.** 그래서 청소하는
자리를 `tasks/<배포>/<내uid>/` 안으로 못 박았습니다.

그 uid 는 반드시 **토큰을 확인한 결과**(`localId`)에서 꺼냅니다. 요청 본문에 적힌
값을 쓰면 남의 칸 이름을 적어 보낼 수 있습니다. 워커 테스트가 이걸 봅니다.

### 알고 받아들인 한계

그림은 **키를 아는 교사면 누구나** 받을 수 있습니다. 워커가 Firestore 를 안 보기
때문에 "이 사람이 이 일정을 공유받았나"를 확인할 방법이 없습니다. 확인하려면
서비스 계정 키를 이 워커에 넣어야 하는데, 그것을 **안 넣는 것**이 이 워커를 따로
판 이유입니다.

덧붙여, `firestore.rules` 에서 `tasks/{uid}/items/{id}` 는 이미
`allow read: if isYnhsTeacher()` 입니다 — **일정 문서 자체가 전 교사에게 열려**
있고, 공유 여부는 화면에서 거르는 것뿐입니다. 그림만 유별나게 새는 것이
아니라 문서와 같은 수준입니다.

## 왜 또 만드나

모의고사 날처럼 **감독 시간·시정이 그날만 바뀌는** 것을 매번 앱에 입력하는 대신,
메신저에 붙여넣듯 **그림을 그대로 붙여** 공지로 띄우려는 것입니다. 그래서 웹주소가
없는 이미지(클립보드 붙여넣기)를 받아야 합니다.

`teacher-api`에 얹지 않은 이유는 위 [왜 나눴나](#왜-나눴나)와 같습니다. 배포가
손으로 붙여넣기라, 공지를 손보다 학생 사진이 죽으면 되돌릴 길이 마땅치 않습니다.

**왜 Firestore가 아니라 R2인가**: 넣을 수는 있지만 문서 하나가 1MiB를 못 넘고,
무엇보다 현황판은 **모든 교사가 매번 여는 화면**입니다. 그림을 문서에 넣으면 그
용량을 매번 다 받습니다. R2에 두면 문서는 글자 몇 KB로 남고, 그림은 따로 받아
브라우저가 일주일치 캐시합니다.
...
| 항목 | 값 |
|---|---|
| 워커 | `notice-api` → `https://notice-api.pkh910518.workers.dev` |
| R2 버킷 | `ynhs-notices` (공개 접근 꺼짐) |
| 바인딩 | `NOTICES` |
| 변수 | `ALLOWED_ORIGINS`, `FIREBASE_API_KEY` |

앱에는 `index.html` 의 `NOTICE_WORKER` 로 이미 박혀 있습니다. 아래 절차는 다시
만들거나 운영(ynhs)에 세울 때를 위해 남겨 둡니다.
...
Cloudflare 대시보드 → **Workers & Pages** → **Create** → *Start with Hello World!*
→ 이름을 **`notice-api`** 로 → **Deploy**

만들어지면 **Edit code** → 기본 코드를 전부 지우고 `notice-api.js` 내용을 붙여넣기
→ **Deploy**
...
|---|---|---|
| Text | `ALLOWED_ORIGINS` | `https://kyunghwanp.github.io` |
| Text | `FIREBASE_API_KEY` | `teacher-api`에 넣은 것과 같은 값 |

> **`SA_JSON`은 필요 없습니다.** 이 워커는 Firestore를 건드리지 않아 서비스 계정이
> 아예 없어도 됩니다. 앞의 두 워커를 세울 때 겪은 '키를 다시 발급받아야 하는' 일이
> 여기서는 없습니다.

### 3) R2 버킷 만들고 붙이기

**R2** → *Create bucket* → 이름 `ynhs-notices`. **공개 접근은 켜지 마세요**
(워커의 토큰 검사를 지나야만 나가야 합니다).

Workers → `notice-api` → **Settings → Bindings → + Add → R2 bucket**

| 항목 | 값 |
|---|---|
| Variable name | `NOTICES` ← 대문자 그대로 |
| R2 bucket | `ynhs-notices` |

> 학생 사진 버킷(`ynhs-photos`)과 **따로** 둡니다. 공지 그림을 잘못 청소하다 학생
> 사진을 지우는 일이 없어야 합니다.
...
```
https://notice-api.pkh910518.workers.dev/img
```

- `{"success":false,"error":"ORIGIN"}` → **정상**입니다(주소창 직접 접속은 허용
  출처가 아니므로 여기서 막힙니다). 워커가 살아 있다는 뜻입니다.
- 아무것도 안 뜨거나 오류 화면 → 배포가 안 된 것입니다.

## 저장 구조

```
notices/<배포>/<공지id>/<파일id>.jpg
   └ test | live      └ 공지 문서 id  └ 앱이 만든 임의 문자열
```

배포별로 앞자리를 가르므로 **test와 운영이 같은 버킷을 봐도 섞이지 않습니다.**
청소(`sweep`)도 자기 배포 아래만 훑습니다.

바깥에서 받은 문자열이 그대로 경로가 되므로, 세 토막 모두 `[A-Za-z0-9_-]{1,40}`
만 통과시킵니다(`../` 같은 것이 애초에 못 들어옵니다).

## 용량

장당 상한 3MB, 화면에서 미리 줄여 보내므로 실제로는 훨씬 작습니다. R2 무료 한도가
10GB라 공지 그림으로는 사실상 걱정할 규모가 아닙니다.

청소(`sweep`)를 부를 때는 **그 배포의 모든 공지가 쓰는 키를 전부** 넘겨야 합니다.
지금 고치는 공지의 키만 넘기면 다른 공지의 그림이 '안 쓰이는 것'으로 보여 통째로
지워집니다. 앱에서는 `noticeAllKeys()` 가 그 일을 합니다.

다만 **공지를 고치거나 지워도 R2 파일은 저절로 안 사라집니다.** 그래서 지우기와
청소 경로를 처음부터 넣어 두었습니다 — 용량이 넉넉해 티가 안 나는 것이 오히려
함정이라, 나중에 붙이면 그때까지 쌓인 것을 손으로 치워야 합니다.

## 안 될 때 보는 곳

앱에서 그림을 붙여넣었을 때 나오는 말로 어디가 막혔는지 알 수 있습니다.

| 화면에 뜨는 말 | 뜻 | 할 일 |
|---|---|---|
| 그림 저장소가 아직 연결되지 않았습니다 | `NOTICES` 바인딩이 없다 | 바인딩 추가 후 **다시 Deploy** |
| 이 그림을 올릴 수 있는 계정이 아닙니다 | 공지인데 관리자 계정이 아니다 | '실제 권한으로 보기'를 껐는지 확인 |
| 그림 저장소가 아직 새 버전으로 바뀌지 않았습니다 | **캘린더 메모**인데 워커가 옛 코드다 | `notice-api.js` 를 다시 붙여넣고 Deploy |
| 보기 모드에서는 그림을 넣을 수 없습니다 | '다른 선생님으로 보기' 중이다 | 보기 모드를 끄고 다시 |
| 로그인이 만료되었습니다 | 토큰 만료 | 새로고침 |
| 연결에 실패했습니다 | 대개 `ALLOWED_ORIGINS` 불일치 | 앱 주소와 **정확히** 같은지 확인 |

가장 흔한 두 가지:

1. **변수를 넣고 Deploy 를 다시 안 눌렀다.** 돌고 있는 버전에는 안 붙습니다.
2. **`ALLOWED_ORIGINS` 가 앱 주소와 다르다.** `https://kyunghwanp.github.io` 처럼
   경로 없이 출처만 적습니다(`/test` 를 붙이지 않습니다). 워커가 끝 슬래시와
   대소문자는 맞춰 주지만 호스트가 다르면 막습니다.

## 되돌리기

바인딩(`NOTICES`)만 지우면 이미지 기능이 조용히 꺼집니다(`NO_BUCKET`). 공지 **글은
그대로 뜹니다** — 글은 Firestore에 있고 워커와 무관합니다.
