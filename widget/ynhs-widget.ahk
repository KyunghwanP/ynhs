...
; ── 전역 초기화 ──────────────────────────────────────────────
; 반드시 RestoreSelected() 보다 위에 있어야 한다.
; WebView2 의 .await() 는 기다리는 동안 Sleep(-1) 로 메시지를 편다. 그래서 두 번째
; 위젯을 만드느라 기다리는 사이에, 이미 만들어진 첫 번째 위젯의 WebMessageReceived
; 가 먼저 실행될 수 있다. 즉 자동 실행 구역이 RestoreSelected() 에 멈춰 있는 동안
; HandleToastMsg 가 돌아 gToastSeen 이 "has not been assigned a value" 로 터졌다.
; (지난번엔 '첫 핫키(#d)보다 앞'까지만 올렸다. 그것만으론 부족하다.)
; 최상위 global 은 슈퍼전역이라 함수들이 선언 없이 쓰는 곳이 여럿이다. 그래서 함수로
; 감싸지 않고 '위치만' 옮긴다.
...
; (BORDER_COLOR·BORDER_COLOR_DARK 는 자동 실행 구역으로 옮김 — 파일 앞쪽 「전역 초기화」 참고)
StyleWindow(hwnd) {
...
; ══════════════════════════════════════════════════════════════
; (토스트·이모지 전역값은 자동 실행 구역으로 옮김 — 파일 앞쪽 「전역 초기화」 참고)
